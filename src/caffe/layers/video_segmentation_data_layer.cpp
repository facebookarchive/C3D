/*
 * video_segmentation_data_layer.cpp
 *
...
void* VideoSegmentationDataLayerPrefetch(void* layer_pointer) {
  CHECK(layer_pointer);
  VideoSegmentationDataLayer<Dtype>* layer = static_cast<VideoSegmentationDataLayer<Dtype>*>(layer_pointer);
  CHECK(layer);
    ...
    if (crop_size) {
      ...
      if (mirror && layer->PrefetchRand() % 2) {
          ...
          for (int l = 0; l < length; ++l) {
            for (int h = 0; h < crop_size; ++h) {
              for (int w = 0; w < crop_size; ++w) {
                int top_index = (((item_id * (channels-1) + c) * length + l) * crop_size + h)
                ...
                if (c < channels-1) {
                	...
                	top_truth[((item_id * length + l) * crop_size + h)
                              * crop_size + (crop_size - 1 - w)] = datum_element;
                }
                if (show_data) {
    	...
    	for (int j = 0; j < size; ++j) {
    		if (j < size - channel_size) {
    			top_data[item_id * size + j] =
    					(data[j] - mean[j]) * scale;
    		} else {
    			top_truth[item_id * channel_size + j - size + channel_size] =
    					data[j];
    		}
    	}
    }
...
template <typename Dtype>
VideoSegmentationDataLayer<Dtype>::~VideoSegmentationDataLayer<Dtype>() {
  JoinPrefetchThread();
}

template <typename Dtype>
void VideoSegmentationDataLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (crop_size > 0) {
    (*top)[0]->Reshape(this->layer_param_.image_data_param().batch_size(),
                       data_blob.channels()-1, data_blob.length(), crop_size, crop_size);
    (*top)[1]->Reshape(this->layer_param_.image_data_param().batch_size(),
                           1, data_blob.length(), crop_size, crop_size);
    prefetch_data_.reset(new Blob<Dtype>(
        this->layer_param_.image_data_param().batch_size(), data_blob.channels()-1,
        	data_blob.length(), crop_size, crop_size));
    prefetch_truth_.reset(new Blob<Dtype>(
            this->layer_param_.image_data_param().batch_size(), 1,
            	data_blob.length(), crop_size, crop_size));
  } else {
    (*top)[0]->Reshape(
        this->layer_param_.image_data_param().batch_size(), data_blob.channels() - 1,
  ...
  if (this->layer_param_.image_data_param().has_mean_file()) {
    ...
    CHECK_EQ(data_mean_.channels(), datum_channels_-1);
    CHECK_EQ(data_mean_.length(), datum_length_);
    CHECK_EQ(data_mean_.height(), datum_height_);
    CHECK_EQ(data_mean_.width(), datum_width_);
  } else {
    // Simply initialize an all-empty mean.
    data_mean_.Reshape(1, datum_channels_-1, datum_length_, datum_height_, datum_width_);
...
template <typename Dtype>
void VideoSegmentationDataLayer<Dtype>::CreatePrefetchThread() {
  phase_ = Caffe::phase();
  ...
  // Create the thread.
  CHECK(!pthread_create(&thread_, NULL, VideoSegmentationDataLayerPrefetch<Dtype>,
        static_cast<void*>(this))) << "Pthread execution failed.";
}

template <typename Dtype>
void VideoSegmentationDataLayer<Dtype>::JoinPrefetchThread() {
  CHECK(!pthread_join(thread_, NULL)) << "Pthread joining failed.";
}

template <typename Dtype>
unsigned int VideoSegmentationDataLayer<Dtype>::PrefetchRand() {
  CHECK(prefetch_rng_);
...
template <typename Dtype>
Dtype VideoSegmentationDataLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) {
